  ...
  if Rails.env.production?
    ...
    if resend_credentials&.dig(:api_key)
      ...
      ActionMailer::Base.rescue_from Resend::Error do |exception|
        Rails.logger.error "Resend Error Details: #{exception.message}"
        Rails.logger.error "Resend Error Class: #{exception.class}"
        Rails.logger.error "Resend Error Backtrace: #{exception.backtrace.first(5).join("\n")}"
        raise exception
      end

      Rails.logger.info "Resend configuration completed successfully"
    else
      Rails.logger.error "No Resend API key found in credentials!"
    end
  end
end
